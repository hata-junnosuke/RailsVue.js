...
<script>
  ...
  export default {
      ...
      methods: {
          // コメントアウトでタグidを1にチェックした時の処理を記録
          async fetchUsers() {
              // フォームに入れた値をsearchParamsへ格納
              const searchParams = {
                  q: {
              ...
              // params = {q:{name: "", tag_ids: [1]},page: 1}
              const paramsSerializer = (params) => qs.stringify(params, { arrayFormat: 'brackets' });
              // qs.stringify(params, { arrayFormat: 'brackets' }) = q%5Bname%5D=&q%5Btag_ids%5D%5B%5D=1&page=1
              const res = await axios.get(`/api/users`, { params, paramsSerializer })//URLにがっちゃんこ
